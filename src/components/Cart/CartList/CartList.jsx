...
function CartList({ id, title, price, imgUrl }) {
  const [totalPriceValue, setTotalPriceValue] = React.useState(1);
  ...
  return (
    <li className={`${style.cartList__item}`}>
      <div className={style.cartList__left}>
        <div className={style.cartList__top}>
          <div className={style.cartList__info}>
            <img className={style.cartList__img} src={imgUrl} alt='products' />
            <div className={style.cartList__desc}>
              <div className={style.cartList__titleWrapper}>
                <h3 title={title} className={style.cartList__title}>
                  {title}.
                </h3>
              </div>
              <div className={style.cardList__price}>
                <span className={style.cardList__priceCurrency}>$</span>
                <span className={style.cardList__priceValue}>{price}</span>
              </div>
            </div>
          </div>

          <div className={style.cartList__top_right}>
            <input
              onChange={totalPrice}
              className={style.cartList__value}
              type='text'
              placeholder='1'
            />
            <div className={style.cartList__totalPrice}>
              <span className={style.cartList__totalPriceCurrency}>$</span>
              <span className={style.cartList__totalPriceValue}>
                {totalPriceValue ? (totalPriceValue * price).toFixed(2) : price}
              </span>
            </div>
          </div>
        </div>
        <div className={style.cartList__bottom}>
          <textarea
            className={style.cartList__note}
            name='note'
            placeholder='Order Note...'
          ></textarea>

          <button className={`btn btn__del ${style.cartList__delBtn}`}>
            <svg
              width='16'
              height='18'
              viewBox='0 0 16 18'
